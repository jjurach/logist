# 09-system-schema.md: System-Level Instructions for LLM Schema Compliance

## Task Overview
This document outlines the development and integration of system-level instructions (`system.md`) to ensure LLM agents produce outputs compliant with predefined JSON schemas, specifically `llm-chat-schema.json`. This task focuses on leveraging `system.md` as a core instruction file, guiding LLMs toward structured responses, and detailing the process for validating these responses and extracting metrics from `cline` oneshot executions.

## Master Plan Requirements
**Description:** Develop and integrate `system.md` for consistent LLM output formatting; define methods for output validation and metric extraction from `cline` tasks.
**Objective:** Ensure all LLM responses adhere to `llm-chat-schema.json` via system instructions and establish a robust process for verifying these outputs and collecting performance metrics.
**Scope:** Creation of `system.md` content, modification of `execute_llm_with_cline()` to accept additional instruction files, definition of output parsing and validation, and metric extraction from `cline` task metadata.
**Dependencies:** `logist/schemas/llm-chat-schema.json`, `job_processor.py` for `execute_llm_with_cline()` and response parsing/validation.
**Files (Read):** `system.md`, `llm-chat-schema.json`, `api_conversation_history.json`, `metadata.json` (from `cline` task directory).
**Files (Write):** `system.md` (content creation), potentially updated LLM response files after validation.
**Verification:** LLM outputs consistently match `llm-chat-schema.json`; metrics are accurately extracted.

## Implementation Sequence
1.  **Draft `system.md` Content:** Create a clear, explicit `system.md` that defines the expected JSON output format.
2.  **Modify `execute_llm_with_cline()` (Conceptual):** Outline the changes to `logist/logist/job_processor.py` to allow `execute_llm_with_cline()` to accept an array of instruction files.
3.  **Define Output Parsing and Validation:** Detail the steps for extracting the valid JSON response from `cline` task output, prioritizing the first valid JSON found in `api_conversation_history.json`.
4.  **Outline Metric Extraction:** Describe how to obtain token usage, cost, and elapsed time from `cline` task metadata.
5.  **Plan Early Iteration Testing:** Suggest explicit prompts for LLMs and verification steps.
6.  **Document and Integrate:** Create this `09-system-schema.md` file to centralize this knowledge.

## Core Implementation Details

### 1. `system.md` Content - Guiding LLM Output
A `system.md` file will contain high-level instructions to LLMs, primarily focusing on ensuring that their final communication adheres to the required JSON schema. This file will be provided to the `cline` command as a `--file` argument.

#### Example `system.md` Content:
```markdown
...
The `execute_llm_with_cline()` function in `logist/logist/job_processor.py` should be enhanced to accept an optional list of `instruction_files`.
...
def execute_llm_with_cline(
    ...
    # Inside, modify the `cline` command construction:
    cmd = [
        "cline", "--yolo", "--oneshot",
...
```
This allows for dynamic inclusion of multiple instruction files (e.g., `system.md`, `worker.json`, `supervisor.json`) in a single `cline` call, enabling layered instruction.
...
3.  **Prioritize First Valid JSON**: For each message, attempt to extract and validate any JSON object that conforms to `llm-chat-schema.json`. The *first valid JSON object found* that matches the schema should be considered the agent's final structured response. This handles cases where the LLM might have conversational text around its JSON, or if it produces multiple JSON blocks in a single `api_conversation_history.json` entry.
...
The extracted JSON response will be passed to existing validation functions in `job_processor.py`, specifically `parse_llm_response` (which extracts JSON) and `validate_llm_response` (which checks against `llm-chat-schema.json`).
...
For early development and testing of `system.md` and the parsing logic:
...
2.  **Execute `cline` Task**: Run the `cline` command with the prompt and the `system.md` file.
3.  **Verify Output**:
    *   Use the parsing strategy described above to extract the JSON from `api_conversation_history.json`.
    *   Validate the extracted JSON against `llm-chat-schema.json`.
