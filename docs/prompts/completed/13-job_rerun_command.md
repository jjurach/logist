# Job Rerun Command Implementation - Cline Oneshot Prompt

## Task Overview
You are implementing the `job_rerun_command` from the Logist master development plan. Consult this prompts file throughout implementation to ensure every requirement is met - reference it explicitly after each major step to verify compliance.
...
3. **State Management:** During a rerun, the job's state will need to be carefully managed. A rerun should ideally create a new "run" entry in the job's history, rather than overwriting the previous run's history, to preserve a clear audit trail. This implies careful interaction with `logist/job_history.py` to record new run attempts. The current active state of the job should reflect the rerun, allowing for its progress to be tracked independently.
...
*   **Job State Dependencies:** Rerun operations should handle cases where intermediate steps may depend on outputs that change between runs. The command should clearly communicate the impact on job state.
*   **Resource Consumption:** Multiple reruns of the same job will accumulate costs and time, so users should be made aware of the resource implications through cumulative metrics tracking.
*   **Concurrent Execution:** If a job is being rerun while another operation is active on the same job, proper locking mechanisms should prevent conflicts and data corruption.
*   **History Accumulation:** Multiple reruns can create extensive history logs, requiring consideration of history pruning strategies and efficient storage mechanisms.
*   **Workspace State Consistency:** Ensure the workspace remains in a reliable state for reruns, as modifications between runs could affect reproducibility and debugging accuracy.
*   **Long-Running Jobs:** Consider time limits and interruption handling for jobs that may run for extended periods during rerun operations.

### Success Criteria for Implementation
- [ ] Command can be invoked as `logist job rerun <job_id> [--step <step_number>]`
- [ ] Validates job_id and optional step_number parameters appropriately with clear error messages
- [ ] Properly resets job state for full reruns or resumes from specific steps while preserving existing history
- [ ] Creates new run entries in job history with clear audit trails for debugging and monitoring
- [ ] Handles error conditions gracefully without corrupting job state or losing previous progress
- [ ] Implements proper resource management and cleanup to prevent accumulation issues
- [ ] Passes all specified unit tests and demonstrates complete functionality in test-demo.sh scenarios

### Related Commands Comparison

| Feature | `job step` | `job rerun` | `job run` | `job restep` |
|---------|------------|-------------|-----------|--------------|
| Execution Scope | Single next step | From specified step to completion | Full job execution | Single specific step |
| Starting Point | Current job phase | Specified step or beginning | Always from start | Single step only |
| History Recording | Appends to current run | Creates new run entry | Initial run creation | Dedicated RESTEP entry |
| State Persistence | Advances phase normally | Full state reset management | Fresh execution state | Preserves overall status |
| Resource Management | Incremental costs | Potential resource accumulation | Baseline measurement | Isolated step costs |
| Use Case | Sequential debugging | Resume interrupted workflows | Initial job execution | Fine-grained step debugging |

### Implementation Quality Guidelines

**Pre-Implementation Checklist:**
- [ ] Review all prerequisite phases and dependencies
- [ ] Analyze existing job execution patterns for integration points
- [ ] Plan state management approach for rerun scenarios
- [ ] Design history recording strategy for multiple run tracking
- [ ] Consider error recovery mechanisms for failed reruns

**Code Quality Requirements:**
- Implement robust error handling with meaningful error messages
- Ensure thread-safety and atomic operations for state modifications
- Add comprehensive logging for debugging and monitoring
- Follow existing code patterns and architectural decisions
- Include input validation and security considerations

**Testing Strategy:**
- Unit tests for all command paths and edge cases
- Integration tests with real job executions
- Performance testing for resource usage tracking
...
**Implementation Notes:** Consult this specification file explicitly after each major implementation milestone to verify 100% requirements compliance. Document any deviations or discoveries that affect the implementation approach.